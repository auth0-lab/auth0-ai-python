# TODO: most part of this implementation should be moved to auth0-ai after updating langchain-auth0-ai
import contextvars
import inspect
from abc import ABC
...
from auth0_ai.authorizers.ciba_authorizer import CIBAAuthorizer as  CIBAAuthorizerBase, CibaAuthorizerOptions
...
async def _run_with_local_storage(data: AsyncStorageValue):
    if _local_storage.get() is not None:
        raise RuntimeError("Cannot nest tool calls that require async user confirmation.")
        ...
        async def wrapped_execute(*args: ToolInput.args, **kwargs: ToolInput.kwargs):
            ...
            async with _run_with_local_storage(store):
                try:
                    ...
                    _update_local_storage({"access_token": credentials["access_token"]})
