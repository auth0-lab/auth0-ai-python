import contextvars
import requests
from typing import Callable, Generic, Optional, Any, TypedDict
from .types import AuthorizerParams, AuthorizerToolParameter, ToolInput, resolve_parameter
...
def get_access_token_for_connection() -> Optional[str]:
...
class FederatedConnectionAuthorizerParams(Generic[ToolInput]):
    def __init__(
        self,
        refresh_token: Optional[AuthorizerToolParameter[ToolInput, str | None]] = None,
        access_token: Optional[AuthorizerToolParameter[ToolInput, TokenResponse | None]] = None,
        scopes: AuthorizerToolParameter[ToolInput, list[str]] = None,
        connection: AuthorizerToolParameter[ToolInput, str] = None,
    ):
...
class FederatedConnectionAuthorizerBase(Generic[ToolInput]):
    ...
    def validate_token(self, token_response: Optional[TokenResponse] = None):
        ...
        if token_response is None:
            raise FederatedConnectionInterrupt(
                f"Authorization required to access the Federated Connection: {self.params.connection}",
                connection,
                scopes,
                scopes
            )
        ...
        if missing_scopes:
            raise FederatedConnectionInterrupt(
                f"Authorization required to access the Federated Connection: {self.params.connection}. Missing scopes: {', '.join(missing_scopes)}",
                connection,
                scopes,
    ...
    async def get_access_token_impl(self, *args: ToolInput.args, **kwargs: ToolInput.kwargs) -> TokenResponse | None:
        ...
        exchange_params = {
            ...
            "client_id": self.auth0["clientId"],
            "client_secret": self.auth0["clientSecret"],
    ...
    async def get_access_token(self, *args: ToolInput.args, **kwargs: ToolInput.kwargs) -> TokenResponse | None:
        if callable(self.params.refresh_token):
            token_response = await self.get_access_token_impl(*args, **kwargs)
        else:
            token_response = await resolve_parameter(self.params.access_token, *args, **kwargs)
        
        self.validate_token(token_response)
        return token_response
    
    async def get_refresh_token(self, *args: ToolInput.args, **kwargs: ToolInput.kwargs):
        return await resolve_parameter(self.params.refresh_token, *args, **kwargs)
    ...
    def protect(
        ...
        async def wrapped_execute(*args: ToolInput.args, **kwargs: ToolInput.kwargs):
            store = {
                "context": await get_context(*args, **kwargs),
                "scopes": await resolve_parameter(self.params.scopes, *args, **kwargs),
                "connection": await resolve_parameter(self.params.connection, *args, **kwargs)
            }
            ...
            try:
                ...
                return self.handle_authorization_interrupts(interrupt)
            except Auth0Interrupt as err:
                return self.handle_authorization_interrupts(err)
